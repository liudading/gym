...
class ClothDryer(object):
    def __init__(self, init_time, dt, rnd=np.random.RandomState()):
        ...
        self.alpha_dist = dict(
            {"mean": (pd.datetime(2016,1,1,21,30) - self.init_time) // self.dt,
             "std": pd.Timedelta(minutes=30) // self.dt,
             "low": (pd.datetime(2016,1,1,21,00) - self.init_time) // self.dt,
             "high": (pd.datetime(2016,1,1,22,00) - self.init_time) // self.dt,
             })
        self.beta_dist = dict(
...
class EV(object):
    ...
    def step(self, action, time):
        ...
        if time >= self.alpha and time < self.beta:
            status = 1.0
            soc += (self.dt.seconds/3600) * action/self.CAPACITY
...
class EWH(object):
    ...
    def _load_data(self):
        ...
        water_usage_power = csv_file['Curve1'].values
...
class SmartHome_ddpg_v1(gym.Env):
    ...
    def reset(self, seed=None, day=None):
        ...
        if time.time() - self._render_time > 0.5 * 60:
            self.render()
        ...
        if day is None:
            self.t = self.rnd.randint(1,62) * self.T
        else:
            self.t = day * self.T
        self.init_time = pd.datetime(2016,1,1,8,0)
    ...
    def _get_reward(self, action):
        ...
        if self.time == self.ev.beta:
            E_range += 10 * ((soc - 1.0) * self.ev.CAPACITY) ** 2
        elif self.time >= self.ev.alpha and self.time < self.ev.beta:
            E_range += (max(0, self.ev.MIN_SOC-soc) * self.ev.CAPACITY) ** 2 + \
                       (max(0, soc-self.ev.MAX_SOC) * self.ev.CAPACITY) ** 2
        E_range = w2 * E_range
